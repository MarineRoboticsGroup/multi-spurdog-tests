#!/usr/bin/env python3
"""
Offline CORA, GTSAM, & EKF (in development) Optimization Script WITH PRIORS

This is a modified version of offline_optimizer.py that adds POSE and LANDMARK priors
to CORA optimization (similar to Morrison's ROS implementation which uses pose priors,
plus landmark priors which CORA supports but Morrison doesn't use in his ROS code).
...
Usage:
    python3 offline_optimizer_with_priors.py <bag_file> [--output-dir <dir>]
    
Example:
    python3 offline_optimizer_with_priors.py tests/pink_solo.bag
    python3 offline_optimizer_with_priors.py tests/pink_solo.bag --output-dir tests/results/pink_solo_priors

# Basic usage (processes entire bag)
    python3 tests/offline_optimizer_with_priors.py tests/pink_solo.bag

# With cutoff timestamp (recommended - excludes ascent data)
    python3 tests/offline_optimizer_with_priors.py tests/pink_solo.bag --cutoff-time 1764048438.567943

# Other validated missions:
    python3 tests/offline_optimizer_with_priors.py tests/past_shep.bag --cutoff-time 1764043197.378492
    python3 tests/offline_optimizer_with_priors.py tests/lush_erle.bag --cutoff-time 1764049657.220171
    python3 tests/offline_optimizer_with_priors.py tests/sour_axle.bag --cutoff-time 1764040146.629958
    python3 tests/offline_optimizer_with_priors.py tests/damp_beer.bag --cutoff-time 1764041947.780546
...
def run_cora_optimization(integrated_state_data, range_data, gps_data):
    """
    Run CORA (batch) optimization on extracted data using actual CORA optimizer WITH PRIORS.
    
    This version adds POSE PRIORS (like Morrison's ROS implementation) and LANDMARK PRIORS
    (which Morrison doesn't use but CORA supports) to test if they improve performance.
    ...
    Priors added:
    - First pose prior: Very tight (1mm position, 0.006° orientation)
    - Landmark priors: Tight (10cm position uncertainty)
    ...
    print(f"\n{'='*80}")
    print(f"RUNNING CORA BATCH OPTIMIZATION WITH PRIORS")
    print(f"{'='*80}\n")
    ...
    # Step 1.5: Add pose prior on first pose (Morrison's approach)
    # This anchors the coordinate frame and prevents arbitrary rotation/translation
    print(f"\nAdding pose prior on first pose...")
    first_state = integrated_state_data[0]
    first_sym = cora.Symbol('A', 0)
    
    # Get first pose rotation and translation
    qw, qx, qy, qz = first_state['qw'], first_state['qx'], first_state['qy'], first_state['qz']
    try:
        first_rot = R.from_quat([qx, qy, qz, qw]).as_matrix()
    except AttributeError:
        first_rot = R.from_quat([qx, qy, qz, qw]).as_dcm()
    first_trans = np.array([first_state['x'], first_state['y'], first_state['z']])
    
    # Create tight covariance for pose prior (anchor the frame firmly)
    # Very tight prior (similar to GTSAM's 1e-6 for position, 1e-8 for orientation)
    pose_prior_trans_std = 1e-3  # 1mm position uncertainty
    pose_prior_rot_std = 1e-4    # ~0.006° orientation uncertainty
    pose_prior_covar = np.diag([pose_prior_trans_std**2] * 3 + [pose_prior_rot_std**2] * 3)
    
    # Add pose prior to problem
    pose_prior = cora.PosePrior(
        id=first_sym,
        R=first_rot,
        t=first_trans,
        cov=pose_prior_covar
    )
    # problem.addPosePrior(pose_prior)
    # print(f"✓ Added pose prior on first pose ({first_trans[0]:.1f}, {first_trans[1]:.1f}, {first_trans[2]:.1f})")
    # print(f"  Prior std: position={pose_prior_trans_std*1000:.1f}mm, rotation={np.rad2deg(pose_prior_rot_std):.4f}°")
    ...
    # Step 2.5: Add landmark priors (NOT in Morrison's ROS implementation, but CORA supports it)
    # This tests whether landmark priors improve CORA's performance
    print(f"\nAdding landmark priors...")
    
    # Tight landmark position priors (we know the beacon positions well)
    landmark_prior_std = 0.1  # 10cm position uncertainty for known landmarks
    landmark_prior_covar = np.diag([landmark_prior_std**2] * 3)
    
    for key_str, position in landmark_positions.items():
        sym = cora.Symbol(key_str[0], int(key_str[1]))
        landmark_prior = cora.LandmarkPrior(
            id=sym,
            p=position,
            cov=landmark_prior_covar
        )
        problem.addLandmarkPrior(landmark_prior)
        print(f"  ✓ Added prior for {key_str} at ({position[0]:.1f}, {position[1]:.1f}, {position[2]:.1f})")
    
    print(f"✓ Added {len(landmark_positions)} landmark priors")
    print(f"  Prior std: {landmark_prior_std*100:.0f}cm position uncertainty")
    ...
    # Print optimization problem summary
    print(f"\n{'='*80}")
    print(f"CORA OPTIMIZATION PROBLEM SUMMARY (WITH PRIORS)")
    print(f"{'='*80}")
    print(f"  Pose variables: {len(pose_keys)}")
    print(f"  Landmark variables: {len(landmark_keys)}")
    print(f"  Total variables: {len(pose_keys) + len(landmark_keys)}")
    print(f"  Pose priors: 1 (first pose)")
    print(f"  Landmark priors: {len(landmark_keys)}")
    ...
    try:
        ...
        # NOTE: With pose priors, the solution should already be close to world frame
        # but we still apply CORA's alignment for consistency with the original script
        ...
        print(f"  (With priors, solution should already be close to world frame)")
...
def main():
    ...
    if args.output_dir is None:
        ...
        args.output_dir = f"tests/results_with_priors/{bag_name}_offline"
